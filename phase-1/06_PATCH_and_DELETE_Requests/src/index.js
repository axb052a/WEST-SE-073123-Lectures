...
function renderBook(book) {
  ...
  inventoryInput.addEventListener("change", (e) => {
    fetch(`http://localhost:3000/books/${book.id}`, {
      ...
      body: JSON.stringify({ inventory: parseInt(inventoryInput.value) }),
    })
      .then((r) => {
        return r.json();
      })
...
storeForm.addEventListener("submit", (e) => {
  ...
  if (storeEditMode) {
    ...
    const id = document.querySelector("#store-selector").value;
    patchJSON(`http://localhost:3000/stores/${id}`, store)
      .then((store) => {
        renderHeader(store);
        renderFooter(store);
      })
